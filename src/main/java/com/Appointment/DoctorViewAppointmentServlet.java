...
@WebServlet("/DoctorViewAppointmentServlet")
public class DoctorViewAppointmentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (users != null) {
			...
			try {
				List<Appointment> appointmentList = AppointmentDBUtil.getDoctorAppointment(uid);

				boolean isTrue = appointmentList.isEmpty();

				if (isTrue == false) {
					request.setAttribute("appointmentList", appointmentList);
					RequestDispatcher dis = request.getRequestDispatcher("DoctorViewAppointment.jsp");
